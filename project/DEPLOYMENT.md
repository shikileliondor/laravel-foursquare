...
| Variable | `FTP_SERVER_DIR` | Chemin FTP du dossier Laravel, par exemple `/foursquare-api` ; le chemin dépend de la racine du compte FTP |

Le serveur est `ftp.studiobeyam.net`, port `21`, en FTPS explicite. Le `.env` local est configuré pour le développement et ne doit pas servir en production. Préparer le secret `PRODUCTION_ENV` avec `APP_ENV=production`, `APP_DEBUG=false`, `APP_URL=https://api.studiobeyam.net` et les identifiants PostgreSQL réels. Conserver la même `APP_KEY` entre deux déploiements.

Dans cPanel, faire pointer le document root du sous-domaine vers `foursquare-api/public`. Le serveur doit disposer de PHP 8.3 ou plus et des extensions requises par Composer. `storage/` et `bootstrap/cache/` doivent être inscriptibles. Après le premier transfert, exécuter `php artisan migrate --force` et `php artisan storage:link` dans le terminal cPanel. Réexécuter les migrations après un déploiement qui en ajoute : FTP ne peut pas lancer de commandes PHP sur le serveur.
